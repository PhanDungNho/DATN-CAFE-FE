...
const Waitorder = () => {
  ...
  const handleReorderClick = (record) => {
    // Điều hướng đến trang hủy đơn hàng
    navigate(`/cancel/${record.id}`);
  };
    ...
    <div>
      <Table
        ...
        pagination={{
          ...
          pageSizeOptions: ["10", "20", "30"], // Các tùy chọn số dòng
      ...
      >
        {selectedRecord && (
          <div>
            <p>
              <strong>Mã hóa đơn:</strong> {selectedRecord.id}
            </p>
            <p>
              <strong>Tên sản phẩm:</strong> {selectedRecord.name}
            </p>
            <p>
              <strong>Giá:</strong> {selectedRecord.major}
            </p>
            <p>
              <strong>Thành tiền:</strong> {selectedRecord.major}
            </p>
            <p>
              <strong>Trạng thái:</strong> {getStatusTag(selectedRecord.active)}
            </p>
            <p>
              <strong>Người đặt:</strong> {selectedRecord.customer}
            </p>
            <p>
              <strong>Ngày đặt:</strong> {selectedRecord.orderDate}
            </p>
          </div>
        )}
      </Modal>
